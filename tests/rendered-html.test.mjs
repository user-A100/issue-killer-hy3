...
test("server-renders the IssuePilot product shell", async () => {
  ...
  assert.match(html, /<title>IssuePilot · 从 Issue 到可执行计划<\/title>/i);
...
test("repository does not ship starter skeleton or hard-coded secrets", async () => {
  ...
  assert.match(page, /IssuePilotApp/);
  assert.match(layout, /IssuePilot/);
